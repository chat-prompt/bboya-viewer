...
> "지금 권장하는 셋업이 왜 이거인지" 맥락이 궁금한 분만. 그냥 셋업부터 들어가고 싶다면 [멀티에이전트 ep.1 (왜 OpenClaw인가)](../multi-agent/ep-02-api-vs-cli)부터 봐도 OK.
...
스레드 매핑·멀티에이전트 라우팅은 ACP 없이도 **route 바인딩 + slack-thread-rehydrate hook + 워크스페이스별 cwd 분리**로 다 처리됨. (자세한 셋업은 [멀티에이전트 ep.2~ep.5](../multi-agent/ep-03-anatomy))
...
### ✅ Claude CLI 방식 (지금 권장 — 멀티에이전트 시리즈 ep.3~ep.5 전제)
...
변천사는 여기까지. 본격 셋업으로 가자 → [멀티에이전트 ep.1 Claude in Slack vs OpenClaw — 우리가 OpenClaw를 쓰는 이유](../multi-agent/ep-02-api-vs-cli)부터 보고 [멀티에이전트 ep.2 작동 흐름](../multi-agent/ep-03-anatomy)으로
