...
> 🛣️ **이 편의 핵심** — *MCP는* `~/.claude/settings.json`에 박으면 OpenClaw가 자동 합쳐(ep.3 참고). *스킬도* 그러겠거니 했는데 **아니다.** 공식문서를 정밀하게 보면 스킬은 OpenClaw 6경로 안에 들어와야만 봇이 본다. Claude CLI 백엔드여도 마찬가지. 셋업은 한 줄이지만, *그 한 줄이 *없으면* 스킬은 안 보인다.
>
> 📜 *MCP 등록 4가지 길* — [ep.3 MCP 등록 길찾기](./ep-03-mcp-options) 먼저 보고 오면 *왜 스킬은 다른지* 한 번에 이해된다.

---

## 🤔 도입 — "MCP는 자동인데 스킬도 자동 아냐?"

뽀피터스 운영하면서 *진짜로 헷갈렸던* 순간. ep.3에서 정리한 *MCP 자동 매핑* 메커니즘 — `~/.claude/settings.json`에 박으면 OpenClaw가 알아서 합쳐. 그래서 *스킬도* `~/.claude/skills/`에 박으면 자동으로 봇이 사용할 거라 믿었다.
...
이 차이가 진짜 헷갈리는 부분이다. ep.3 MCP 정책과 비교:
...
> 📜 *MCP 등록 길찾기* — [ep.3 MCP 등록 길찾기](./ep-03-mcp-options)
> 📜 *PI vs Claude CLI 큰 그림* — [ep.2 PI vs Claude CLI](./ep-02a-pi-vs-cli)
