...
이번 편 빌링 락은 *fail 처리 시간*만 줄였다. 그런데 진짜 burst rate-limit이 *왜 자주 뜨는지*는 안 풀었다. 메인 게이트웨이가 cron + hook + 슬랙 + 카톡을 같은 OAuth로 동시에 두드릴 때, Anthropic이 *동시 활성 세션 한도*를 어떻게 잡는지 — 그건 다음 편 어디선가.
