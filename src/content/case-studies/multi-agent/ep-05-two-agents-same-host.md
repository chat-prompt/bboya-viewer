...
> 💡 **비유 — 내부 DM 통로** — `subagents.allowAgents`는 **같은 머신 동료끼리만의 사적 통로**야. 슬랙 채널엔 안 보이고 둘만 주고받음. 다른 머신엔 못 닿음. 즉 *내부 DM 같은 개념*.
>
> 흥미롭게도 뽀피터스는 이 통로를 **의도적으로 안 써**. 같은 머신 뽀야↔뽀짝이도 슬랙 채널로 협업해. 왜 그렇게 결정했는지는 → [ep.5 N대 머신](./ep-06-multi-hosts)에서 풀어줄게. ep.4 본판은 *원리*만 잡고 가자.
...
> 🎯 **이 편의 작동법 = 혼자 여러 마리 부리는 효율 워크플로**
> 같은 머신 안에서 `sessions_send`·위임 spawn 같은 *내부 통로*를 풀로 활용하는 패턴. 봇 여러 마리를 *내가 부려쓰는 도구*처럼 굴릴 때 가장 강력해.
>
> 반면 **여러 사람이 봇을 독립 팀원으로 받아들이는 조직형 협업**은 양상이 달라 — 의존성을 의도적으로 분산하고, 협업을 슬랙 한 곳으로 통일해. 그 갈라지는 지점은 → [ep.5 N대 머신](./ep-06-multi-hosts)에서. 본 편은 *혼자 여러 마리* 패턴을 충실히 풀고 가자.
