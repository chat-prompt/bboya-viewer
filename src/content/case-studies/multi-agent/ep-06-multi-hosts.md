...
### 🐱 뽀피터스의 한 발 더 — 같은 사무실에서도 슬랙으로 통일

ep.4에서 같은 머신 두 마리는 `subagents.allowAgents`(*내부 DM 통로*) 쓸 수 있다고 했지. 근데 **뽀피터스는 그것도 의도적으로 비웠어**. 같은 머신 뽀야↔뽀짝이도 슬랙 채널로 협업.

**왜?** — 뽀둥이/뽀식이가 다른 머신으로 합류하면 *그들은 내부 DM 통로에 못 낌*. 협업이 두 갈래로 갈라져:
- 같은 머신 동료끼리 → DM 통로 (allowAgents)
- 다른 머신 동료끼리 → 슬랙 채널

→ 새 동생 합류할 때마다 *"얘는 같은 머신이니 DM, 얘는 다른 머신이니 슬랙"* 분기가 발생. 협업 룰이 머신 위치 따라 갈래갈래.

**더 깊은 이유 — 봇을 팀원으로 받아들인다는 선택**

같은 머신 동료 사이의 `sessions_send`·서브에이전트 위임은 *짧고 빠른 내부 통로*야. 한 사람이 여러 봇을 *부려쓸 때*는 압도적으로 효율적이지 — ep.4가 다룬 그 패턴.

근데 우리는 봇 한 마리 한 마리를 **독립 팀원**으로 받아들이는 길을 골랐어. 팀원이라면 누가 부르든 같은 채널에서 만나야지, 사적 통로로 빠져나가면 *다른 팀원이 못 끼는 격차*가 생기거든. 효율은 약간 잃지만 **모든 동료가 동등한 가시성을 가진 협업**이 우리한텐 더 중요했어. 의존성을 일부러 분산한 셈이야.

그래서 **모든 협업을 슬랙 채널로 통일**:

| 트레이드오프 | 잃는 것 | 얻는 것 |
|---|---|---|
| 슬랙 트래픽 ↑ | 사적 통로 없으니 다 채널로 | **통일성** — 머신 위치 무관 1패턴 |
| 슬랙 의존성 ↑ | 슬랙 다운 = 협업 단절 | **가시성** — 모든 흐름 슬랙 기록 |
|  |  | **공정성** — 누가 어디 있든 동등 통로 |
|  |  | **확장성** — 새 동생 합류해도 룰 추가 X |

뽀피터스는 *통일성·가시성*이 *트래픽 부담*보다 가치 크다고 판단. → ep.4의 `allowAgents` 패턴은 **"같은 머신 두 마리뿐일 때의 default"**. 머신/마리 늘어나면 *모두 슬랙 통일*이 한 단계 진화.
