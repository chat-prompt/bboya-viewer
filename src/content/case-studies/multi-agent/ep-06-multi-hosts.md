...
> 🎯 **이 편의 진짜 핵심 = 여러 사무실이 어떻게 한 팀처럼 일하나**
> 머신을 N대로 나눈 건 *외형*. 안에 박힌 **공유 자산 + 운영 규칙 + 스킬 위치**가 본판이야. 머신·게이트웨이 분리는 *왜 나누는지* 별도 섹션에서 짧게 다루고, 본문은 *공유와 규칙*에 시간 더 써.

같은 슬랙에 4마리 봇이 다른 머신에서 출근해 있는 분산 구조를 셋업하는데, 셋업 자체보단 **운영 규칙**이 본질. 4가지 축:

1. 📚 **공용 자료실** — `bbopters-shared` git 레포에 스킬·hook·팀 문서·페르소나 템플릿. 모든 머신이 clone, 작업 전 `git pull`
2. 📐 **공유 vs 분리 규칙** — 텍스트성 공통 자산(스킬·hook·문서)은 git 공유. 상태·정체성·인증(MEMORY·OAuth·`openclaw.json`)은 머신·에이전트별 독립
...
4. 🤝 **협업 규칙** — 머신 걸친 협업은 슬랙 스레드를 메시지 큐처럼. `subagents.allowAgents`는 같은 사무실 안만 — 우린 이마저도 의도적으로 안 씀(아래)

머신·게이트웨이 분리 *이유*는 부수 정보로 → [왜 사무실을 여러 곳으로 나누나?](#왜-사무실을-여러-곳으로-나누나) / [진화 단계](#-진화-단계--한-머신-안에서-게이트웨이-나누기)
...
> 💡 **두 가지 분리는 다른 레이어** — 머신을 나누는 이유(여기)와 *같은 머신 안에서 게이트웨이를 나누는 이유*(아래 [진화 단계](#-진화-단계--한-머신-여러-게이트웨이))는 다른 문제를 푸는 분리야. 머신은 *양육자·물리 인프라*, 게이트웨이는 *프로세스 격리·EMFILE·OAuth*.

### 🥇 핵심 이유 — 양육 구조

- **👥 양육자 분리**: 봇은 *각자 양육자*가 키우는 팀원. 뽀야·뽀짝이는 집사, 뽀둥이는 소파, 뽀식이는 타타. 양육자가 다르면 봇도 *그 양육자 머신*에 살아야 함 — 봇 = 자기 양육자 옆에서 자라는 직원
- **🧑‍💻 메인 양육자 병목 회피**: 한 명이 혼자 여러 봇 키울 때 *전부 한 머신에 묶이면* 터미널 조작·디버깅·세션 모니터링 시 모든 봇이 한 화면에 몰려 병목. 머신을 나눠야 동시에 굴러감
...
### 🥇 공용 자료실이 왜 필요한가 — 흩어졌어도 같은 자료를 봐야 같은 팀

흩어진 사무실의 봇들이 *같은 룰·같은 도구·같은 팀 정보*를 봐야 한 팀이 됨. 매 머신마다 따로 관리하면:
- 룰 드리프트 (한쪽은 옛 정책, 한쪽은 새 정책)
- 스킬 버전 불일치 (한쪽은 옛 스키마로 작업해서 사고)
- 팀 정보 모름 (새 동생 합류했는데 한쪽이 인지 못 함)

→ **공용 git 레포(`bbopters-shared`)** 한 곳에 스킬·hook·팀 문서·정책 다 두고, 모든 머신이 git pull로 동기화. *한 곳에서 update + 모두 git pull* — 이게 분산 시스템에서 일관성을 지키는 가장 단순한 길.

### 🥈 왜 봇끼리 내부 대화(`allowAgents`)를 안 쓰나 — 격차 안 만들기

같은 머신 봇끼린 `subagents.allowAgents`로 인터폰 직통 대화 가능. *효율적*이지만 다른 머신에 사는 봇은 *그 인터폰에 못 낌*. 한 명이라도 다른 방에 있으면 협업이 *두 갈래*로 갈라져:

- 같은 방 안: 인터폰 (allowAgents) — 빠르고 사적
- 다른 방 넘으면: 슬랙 채널 — 공개 가시성

새 동생 합류할 때마다 *"얘는 같은 방이니 인터폰, 얘는 다른 방이니 슬랙"* 분기 룰 추가. 협업 패턴이 머신 위치 따라 흐트러짐 → 누군 끼고 누군 못 끼는 *격차*.
...
> 📬 **비유** — 같은 *건물(슬랙 워크스페이스)*에 방이 여러 개. 뽀야·뽀짝이는 *한 방에 같이*(같은 머신), 뽀둥이는 *자기 방* 따로, 뽀식이도 *자기 방* 따로. 같은 방 안 책상끼리는 인터폰(`subagents.allowAgents`)으로 직접 부를 수 있는데, *방을 넘어가면 인터폰 안 닿아*. 그땐 *건물 공용 소통 채널(슬랙)*로 보내야 닿음.
...
│ │ 게이트웨이 1     │ │ │ .json      │ │ .json      │
│ │ (뽀야)           │ │ │ agents:    │ │ agents:    │
│ │ openclaw-bboya/  │ │ │  bboongi(d)│ │  bbosiki(d)│
│ │  .json           │ │ │            │ │            │
│ │  agents:         │ │ │ bindings:  │ │ bindings:  │
│ │   bboya (d)      │ │ │  bboongi→  │ │  bbosiki→  │
│ └──────────────────┘ │ │            │ │            │
│ ┌──────────────────┐ │ │            │ │            │
│ │ 게이트웨이 2     │ │ │            │ │            │
│ │ (뽀짝이)         │ │ │cli-backend │ │cli-backend │
│ │ openclaw.json    │ │ │ warm stdio │ │ warm stdio │
│ │  agents:         │ │ │ × 1        │ │ × 1        │
│ │   bbojjak (d)    │ │ │            │ │            │
│ └──────────────────┘ │ │            │ │            │
│                      │ │            │ │            │
...
└──────────────────────┘ └────────────┘ └────────────┘
   │                       │              │
   ▼                       ▼              ▼
 claude × 2 (게1·게2)    claude          claude
  workspace-bboya         workspace-     workspace-
  workspace-bbojjak       bboongi        bbosiki
```
...
## 🐱 진화 단계 — 한 머신 안에서 게이트웨이 나누기

> 💡 **머신 분리와 다른 레이어** — 머신 분리(위)는 *양육자·물리 인프라* 분리. 이 섹션은 **같은 머신 안에서 게이트웨이를 나누는** 또 다른 레이어. 한 양육자가 한 머신에서 봇 여러 마리 키울 때, 봇이 늘면서 *한 게이트웨이 부담*이 커질 때 적용해.

뽀피터스 실제 구조에선 두 분리가 **함께 일어나**. 머신은 양육자가 다른 만큼 나누고, 한 양육자(집사) 머신 안에서도 게이트웨이를 또 나눈 거지. 이번 섹션은 *후자(같은 머신 안 분리)*만 다룸.
...
각 게이트웨이는 **별도 launchd 프로세스 + 별도 config 디렉토리**. 그래서:

| 분리 효과 | 풀어 쓰면 |
|---|---|
| 🛡️ **장애 격리** | 한 게이트웨이 사고 = 그 직원만 쉼. 다른 직원은 멀쩡 |
...
| 👥 **팀원 독립성** | 각자 자기 인프라를 가진 *독립 직원*. 정체성도 인프라도 분리 |

본판 ep.5의 *머신 N대* 패턴과 다른 점: 머신은 1대인데 **논리적 분리**만 N개. 비용 한 대분(전기·관리)인데 격리 효과는 N대분.

### 이게 본판 ep.4 vs 본판 ep.5 vs 진화 단계 — 의존성 스펙트럼

| | 본판 ep.4 (같은 머신·1게이트웨이) | 본판 ep.5 (N대 머신·N게이트웨이) | 진화 단계 (1머신·N게이트웨이) |
|---|---|---|---|
| **의존성 분산** | 낮음 (한 게이트웨이 사고 = 전원 정지) | 가장 높음 (장애·자원·네트워크 다 격리) | 중간 (장애·자원·OAuth 격리, 머신은 공유) |
| **내부 통로(`sessions_send`·위임 spawn)** | 활용 가능 (효율 ↑) | 머신 걸쳐 불가 → 슬랙 통일 | 게이트웨이 걸쳐 불가 → 슬랙 통일 |
| **운영 비용** | 가장 낮음 | 높음 (머신 N대) | 중간 (머신 1대) |
| **추천 시점** | 봇 1~2마리 시작 단계 | 머신 추가 부담 가능, 도메인 격리 필요 | 봇 늘면서 한 게이트웨이 부담 ↑ + 머신 추가는 부담 |

> 💡 **언제 진화 단계로 가나** — 머신 추가는 부담스러운데 봇이 늘어나면서 한 게이트웨이 부담이 커질 때. EMFILE이나 메모리 폭증으로 한 봇 사고가 다른 봇으로 번지는 순간이 *분리 트리거*야.
