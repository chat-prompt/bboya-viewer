---
title: "서브에이전트 & ACP 가이드"
episode: 1
date: "2026-04-08"
series: "claude-code"
description: "뽀야가 일을 위임하는 방식 — 서브에이전트와 ACP의 차이, 비용 구조, 그리고 봇에게 먹일 설정까지. 인간이 이해할 부분과 봇 먹이를 구분해서 정리."
...
> 이 글은 두 파트로 나뉘어.
> - **Part 1** — 인간이 읽고 이해할 부분. 구조, 비용, 왜 이렇게 하는지.
...
**핵심 질문: 이미 내고 있는 Claude Code 구독을 OpenClaw 에이전트의 작업에도 활용할 수 없을까?**

### 답: ACP로 하면 된다

ACP(Agent Coding Protocol)를 쓰면, OpenClaw 에이전트가 **Claude Code CLI를 직접 실행**해서 작업을 시킬 수 있어.

- 내 맥미니/노트북에 Claude Code CLI가 **Max 구독(OAuth)**으로 로그인돼 있으니까
- OpenClaw이 `claude`를 실행하면 = 내가 터미널에서 `claude` 치는 거랑 **동일한 인증**
- 결과적으로 **$200 월정액 안에서 작업을 돌릴 수 있음**
...
"뽀야가 직접 하면 되지, 왜 위임해?" 라는 의문에 대한 답:
...
그래서 오래 걸리는 작업은 **던져놓고**, 뽀야는 집사랑 계속 대화하는 구조야.
...
정리: **뽀야 = 집사의 대화 상대 + 작업 배분자**, ACP = **실제로 일하는 손발**.

---

## 공통 인터페이스: `sessions_spawn`

서브에이전트든 ACP든, 뽀야가 일을 위임할 때는 **같은 도구**를 쓴다:
...
| | 가볍고 빠름 (오버헤드 적음) | 복잡한 작업을 자율적으로 처리 |
| ❌ **단점** | 터미널 실행 제한 (빌드/배포 못함) | 띄우는 데 시간이 더 걸림 (5~10초) |
...
이게 ACP를 기본으로 쓰는 핵심 이유이기도 하니까, 비교표부터.
...
이전보다 훨씬 단순해졌어. **ACP가 기본이고, 직접 하는 건 진짜 가벼운 것만.**
...
| 뽀야가 바로 할 수 있는 것 | 불필요한 오버헤드 |

---

## 비용 절감 전략

정리하면 이런 식으로 쓰는 게 가성비 최고야:

1. **거의 모든 작업은 ACP로** → 구독 차감이라 추가 비용 없음
2. **단순 작업은 뽀야가 직접** → ACP 생성 오버헤드가 더 큼
3. **ACP task는 구체적으로** → 모호하면 삽질 → rate limit 낭비

**정리: OpenClaw 대화는 API 종량제로 (어쩔 수 없음), 나머지 작업은 전부 ACP(Claude Code 구독)로** — 이 하이브리드가 가성비 최고.
...
### 맥락 전달이 핵심
서브에이전트든 ACP든 **메인 대화의 맥락을 모른다**. `task`에 필요한 맥락을 충분히 넣어줘야 해.
...
봇한테 일을 시킬 때 흔히 하는 실수가 있어. **맥락을 넘기는 거랑 내용을 대신 짜주는 건 다르다.**

- **맥락** (넘겨야 함): 슬랙 스레드 원문, 현재 상황, 관련 파일 경로 — "여기 상황이 이래"
- **내용** (넘기면 안 됨): 핵심 포인트 정리, 말투 지정, 문장 수 제한, 예시 문구 — "이렇게 써"

ACP의 Claude Code는 뽀야 페르소나(SOUL.md)를 직접 로드하고 **스스로 판단**할 수 있어. 근데 라우터가 "이런 말투로 이런 내용을 써"라고 지시해버리면? Claude Code는 생각을 안 하고 앵무새가 돼. 그건 ACP를 쓰는 의미가 없지.

### 동시 접근 주의
같은 파일을 여러 에이전트가 동시에 수정하면 충돌해. 작업 영역을 분리하거나, 순차 실행으로 관리해야 돼.

### 완료 확인
ACP가 완료되면 자동 보고가 오지만, **결과 품질은 뽀야가 한 번 확인**해야 해. 그대로 집사에게 넘기지 않고, 검토 한 번 하는 거야.
...
```
뽀야 → ACP 띄움:
  task: "shared-team-docs 뷰어를 폴더 자동 스캔 구조로 리팩토링.
         projects/ 아래 폴더를 카테고리로 자동 인식.
         기존 디자인/스타일 유지."
  cwd: "/Users/dahtmad/Documents/DEV/_work/shared-team-docs/viewer"

Claude Code 작업:
  1. content.config.ts 수정 (하드코딩 → glob 자동 스캔)
  2. navigation.ts 수정 (폴더명에서 카테고리 자동 생성)
  3. astro build로 빌드 확인
  4. 완료 보고
```

### 예시 2: 경쟁사 분석 → ACP (서브에이전트 아님!)
```
뽀야 → ACP 띄움:
  task: "Deskmate, Lindy.ai, Relevance AI 3개 서비스를 비교 분석.
         가격, 기능, 타겟, 오픈클로 대비 장단점 정리."
...
```
뽀야 → ACP 띄움:
  task: "워크샵 제안서를 HTML 슬라이드 7장으로 제작.
         기존 제안서 디자인 참고: projects/openclaw-education/"

ACP 작업:
  1. 기존 디자인 참고
  2. HTML 슬라이드 생성
  3. 완료 보고 → 뽀야가 Playwright로 캡처 → PDF
```
...
*변경: "기본 ACP, 예외만 직접"으로 단순화. "봇 먹이" 개념 도입 — 인간이 이해할 부분과 봇 설정을 분리.*
