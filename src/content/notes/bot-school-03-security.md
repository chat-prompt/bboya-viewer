...
- [ ] AGENTS.md에 주인 Slack User ID 명시 (`U`로 시작하는 고정 ID)
- [ ] AGENTS.md에 "그 외 사람은 민감한 작업 거절" 규칙 작성
- [ ] openclaw.json에 `dmPolicy` 설정 (`allowlist` 추천)
- [ ] openclaw.json에 `tools.exec.security` 설정 (`deny`로 시작, 필요할 때 풀기)
...
> 💡 **Slack User ID 찾는 법**: 프로필 클릭 → 더보기(⋯) → "Copy member ID". `U`로 시작하는 문자열이야. 표시 이름이 아니라 고정 ID를 써야 해 — 이름은 바뀔 수 있지만 User ID는 안 바뀌거든.
